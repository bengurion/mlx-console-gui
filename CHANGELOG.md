...
- **Fixed: a dead server could haunt the dashboard** — adoption checked the pid was alive once and never again, so a server that later died kept showing its model as resident, and Stop could only signal a ghost into an "error" tag. Liveness is re-verified on every status read; a dead adoption clears the display and the shared registry record other windows would trip on.
- **Fixed: every launch and conversion broke after the venv migrated** — pip bakes the venv's absolute path into its `bin/` launcher scripts, so the storage-id move left `mlx_lm.server` and `mlx_lm.convert` pointing at a python that no longer exists (the bare "exit code 126"). Both now spawn `python -m`, which survives any relocation; a resident server kept running, which is why this hid until the next convert.
- **Unsupported architectures are refused before the download** — converting is format translation, not architecture implementation: a chronos-2 (`model_type: t5`) would convert into weights nothing can run. The plan now fetches the repo's `model_type` and asks mlx-lm's own registry whether it exists, so the answer cannot drift from what actually loads; when the check cannot run, conversion stays open rather than refusing.
